...
const calculateShippingFee = async (req, res, next) => {
	try {
		...
		let sellerDistrictId = from_district_id;

		// Nếu Frontend không gửi from_district_id, Backend tự tìm theo product_id
		if (!sellerDistrictId && product_id) {
			const product = await prisma.product.findUnique({
				where: { ID: Number(product_id) },
				include: { Seller: { include: { User: true } } },
			});
			sellerDistrictId = product?.Seller?.User?.ghn_district_id;
		}

		// Fallback nếu vẫn không có
		if (!sellerDistrictId) sellerDistrictId = 1454; // Quận 12

		// Giả sử chọn gói dịch vụ đầu tiên (thường là gói chuẩn/rẻ nhất)
		const service_id = services[0].service_id;

		// 2. Gọi API tính phí và tính expected date
		const [feeData, leadTimeDate] = await Promise.all([
			// Gọi API Tính tiền
			ShippingService.calculateFee({
				service_id: service_id,
				insurance_value: Number(insurance_value),
				from_district_id: Number(sellerDistrictId),
			...
			// Gọi API Tính giờ
			ShippingService.calculateExpectedDelivery(
				Number(sellerDistrictId),
		...
		res.status(200).json({
			...
			expected_delivery_time: leadTimeDate, // GHN có trả về thời gian dự kiến
		});
	} catch (error) {
		next(error);
	}
